...
export async function listAlerts(
  shopId,
  { severity, category, status = 'ACTIVE', page = 1, pageSize = 20, historyDays = null } = {}
) {
  const where = { shopId };

  // Plan-limited alert history. Older alerts are retained, just not listed.
  if (historyDays) {
    where.lastDetectedAt = { gte: new Date(Date.now() - historyDays * 24 * 3600 * 1000) };
  }
...
export async function getAlertFacets(shopId, { historyDays = null } = {}) {
  const window = historyDays
    ? { lastDetectedAt: { gte: new Date(Date.now() - historyDays * 24 * 3600 * 1000) } }
    : {};
  const active = { shopId, status: { in: ['OPEN', 'ACKNOWLEDGED'] }, ...window };

  const [bySeverity, byCategory, resolved] = await Promise.all([
    prisma.alert.groupBy({ by: ['severity'], where: active, _count: { _all: true } }),
    prisma.alert.groupBy({ by: ['category'], where: active, _count: { _all: true } }),
    prisma.alert.count({ where: { shopId, status: 'RESOLVED', ...window } }),
  ]);
