...
export async function evaluateVariant(store, { product, variant, previousQuantity, settings }) {
  ...
  if (current <= 0 && previousQuantity > 0) {
    ...
    return {
      ...
      // Exposure is one week of the demand this variant actually had. Nothing
      // is claimed when there is no sales history to base it on.
      valueAtRisk: Number((impact.estimatedDailyRevenueAtRisk * 7).toFixed(2)),
      metadata: {
