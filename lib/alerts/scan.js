...
import { evaluateProfitLeaks, PROFIT_ALERT_TYPES } from '@/lib/alerts/rules/profitAlerts';
import { evaluateDuplicates, DUPLICATE_ALERT_TYPES } from '@/lib/alerts/rules/duplicateAlerts';
...
export async function runAlertScan(store, { notify = true } = {}) {
  ...
  // Catalogue hygiene is available on every plan: a duplicated product costs
  // the merchant money regardless of what they pay us.
  if (settings.productAlertsEnabled) {
    definitions.push(...(await evaluateDuplicates(store)));
    scannedTypes.push(...DUPLICATE_ALERT_TYPES);
  }
  if (settings.salesAlertsEnabled && hasFeature(store, FEATURES.ADVANCED_ALERTS)) {
    definitions.push(...(await evaluateCustomerHealth(store)));
    scannedTypes.push(...CUSTOMER_ALERT_TYPES);
  }
  if (settings.profitAlertsEnabled && hasFeature(store, FEATURES.PROFIT_ALERTS)) {
    definitions.push(...(await evaluateProfitLeaks(store)));
    scannedTypes.push(...PROFIT_ALERT_TYPES);
  }
