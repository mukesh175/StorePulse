...
export async function seedDemoStore({ reset = true } = {}) {
  ...
  // Realistic Indian D2C assumptions so the profit report has something to work with.
  await prisma.costSetting.upsert({
    ...
    create: {
      ...
      codRtoPercent: 18,
      codRtoCostPerOrder: 180,
      monthlyAdSpend: 60000,
    ...
    for (let v = 0; v < 5; v += 1) {
      ...
      await prisma.productVariant.create({
        data: {
          ...
          // Cost between 45% and 80% of price, so some products are healthy
          // and some are genuinely unprofitable once returns are counted.
          unitCost: Math.round(price * (0.45 + rand() * 0.35)),
  ...
  for (let i = 0; i < 50; i += 1) {
    ...
    const usesDiscount = rand() > 0.55;
    const discountCode = pick(rand, ['WELCOME10', 'FLAT30', 'FESTIVE20']);
    const discountAmount = usesDiscount
      ? Math.round(total * (discountCode === 'FLAT30' ? 0.3 : discountCode === 'FESTIVE20' ? 0.2 : 0.1))
      : 0;
    const zone = pick(rand, [
      ['IN', 'MH'],
      ['IN', 'DL'],
      ['IN', 'KA'],
      ['IN', 'WB'],
    ...
    const order = await prisma.order.create({
      data: {
        ...
        // Free shipping above 1500, else a flat 79 charged to the customer.
        shippingCharged: total >= 1500 ? 0 : 79,
        discountCodes: usesDiscount ? [discountCode] : [],
        shippingCountry: zone[0],
        shippingProvince: zone[1],
        orderNumber: `#${10450 + i}`,
