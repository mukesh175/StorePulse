...
export async function dispatchAlertNotification(store, alert) {
  ...
  // Free plan gets the daily digest; instant email is a paid entitlement.
  if (!hasFeature(store, FEATURES.INSTANT_EMAIL)) return { skipped: true, reason: 'plan' };
...
export async function dispatchWeeklySummary(store, summary) {
  ...
  if (!hasFeature(store, FEATURES.WEEKLY_SUMMARY)) return { skipped: true, reason: 'plan' };
