...
import { logCustomerDataImported } from '@/lib/audit';
...
export function mapVariant(node) {
  return {
    ...
    // Null when the merchant never entered a cost — margin stays unknown
    // rather than being silently treated as 100%.
    unitCost: node.inventoryItem?.unitCost?.amount != null ? toDecimal(node.inventoryItem.unitCost.amount) : null,
...
export function mapOrder(node, storeCurrency) {
  ...
  return {
    ...
    totalDiscounts: toDecimal(node.currentTotalDiscountsSet?.shopMoney?.amount),
    shippingCharged: toDecimal(node.totalShippingPriceSet?.shopMoney?.amount),
    discountCodes: (node.discountApplications?.nodes ?? []).map((d) => d.code).filter(Boolean),
...
export async function syncOrders(store, { max = 250, days = 60, deadline = null } = {}) {
  ...
  // Orders carry customer name and email — record that they entered the system.
  if (written > 0) await logCustomerDataImported(store.id, written);

  return { count: written, total: nodes.length, complete: written === nodes.length };
}
