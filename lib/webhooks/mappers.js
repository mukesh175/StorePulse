...
export function restOrderToRecord(payload, storeCurrency) {
  ...
  return {
    ...
    totalDiscounts: num(payload.current_total_discounts ?? payload.total_discounts),
    shippingCharged: (payload.shipping_lines || []).reduce((s, l) => s + num(l.price), 0),
    discountCodes: (payload.discount_codes || []).map((d) => d.code).filter(Boolean),
    shippingCountry: payload.shipping_address?.country_code ?? null,
    shippingProvince: payload.shipping_address?.province_code ?? null,
