...
import { planIdFromSubscriptionName } from '@/lib/billing';
...
async function handleAppUninstalled(store) {
  // Shopify cancels any active subscription when an app is uninstalled, so the
  // local plan must be cleared with it. Leaving it set means a reinstall
  // silently restores a paid plan the merchant is no longer being charged for.
...
/**
 * Shopify notifies us whenever a subscription changes state — approved,
 * declined, cancelled, expired or frozen. Reacting to it keeps the local plan
 * honest without waiting for the merchant to open the app.
 */
async function handleAppSubscriptionUpdate(store, payload) {
  ...
  const planId = active ? planIdFromSubscriptionName(subscription.name) : 'FREE';
