...
async function handleCustomerDataRequest(store, payload) {
  ...
  // hand over, so this is an audited no-op (the WebhookEvent row is the record).
  console.info(
    `[storepulse] customers/data_request for shop ${store.shopDomain}, customer ${payload?.customer?.id ?? 'unknown'}`
  );
}
